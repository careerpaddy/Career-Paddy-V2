...
class CareerPaddyV2SupabaseUser extends BaseAuthUser {
  CareerPaddyV2SupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> careerPaddyV2SupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CareerPaddyV2SupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
